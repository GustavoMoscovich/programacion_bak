...
router.post("/", (req, res) => {
  ...
  cart.id =
    arrayCarts.length === 0 ? 1 : arrayCarts[arrayCarts.length - 1].id + 1;
...
router.post("/:cid/product/:pid", (req, res) => {
  ...
  if (cartIndex !== -1) {
    const prodIndex = arrayCarts[cartIndex].products.findIndex(
      (prod) => prod.id === prodId
    );
    if (prodIndex !== -1) {
      // el ID del producto existe en el carrito entonces se incrementa la cantidad
      arrayCarts[cartIndex].products[prodIndex].quantity =
        arrayCarts[cartIndex].products[prodIndex].quantity + 1;
      cartManager.addToFile(JSON.stringify(arrayCarts, null, "\t"));
      res.send({
        status: "success",
      ...
      // el ID del producto NO existe en el carrito entonces se agrega como nuevo con cantidad=1
